 ...
 *
 * generator-barephp
 * https://github.com/stormiix/generator-barephp
 *
...
var BarePHP = module.exports = function BarePHP() {
	...
	this.defaults = {
		...
		config: {
			...
			dirSrc: 'src',
			dirSrcSubFolders: ['App', 'App/Controllers', 'App/Models', 'App/Views', 'App/Views/Home', 'Core', 'logs', 'cache'],
			dirTests: 'tests',
			dirBuild: 'build',
			dirPublic: 'public',
	...
	if (!shell.which('git')) {
		...
		this.defaults.owner.name = _.clean(shell.exec('git config user.name', {
			silent: true
		}).output, '\n');
		this.defaults.owner.email = _.clean(shell.exec('git config user.email', {
			silent: true
		}).output, '\n');
		this.defaults.owner.homepage = _.clean(shell.exec('git config user.homepage', {
			silent: true
		}).output, '\n');
	}
...
BarePHP.prototype.welcome = function() {
	this.log(
		yosay('\'Allo \'allo!\n This is a custom barePHP generator version by Stormix.co')
	);
};
	...
	var prompts = [{
		...
		},
		{
			...
			default: this.config.get('accountRepository') ?
				this.config.get('accountRepository') : this.defaults.owner.canonical
		}
	];
...
BarePHP.prototype.askForProject = function() {
	...
	var prompts = [{
		...
		default: this.config.get('projectName') ?
			this.config.get('projectName') : _.cleanDiacritics(process.cwd().split(path.sep).pop()).replace(/\s+/g, '_')
	}];
...
BarePHP.prototype.askForToolsInstall = function() {
	...
	this.prompt(prompts, function(props) {
		var hasMod = function(mod) {
			return props.tools.indexOf(mod) !== -1;
		};
...
BarePHP.prototype.askForPackagistAccount = function() {
	...
	var prompts = [{
		...
		default: this.config.get('controlRepository') ?
			this.config.get('accountPackagist') : this.defaults.owner.canonical
	}];
...
BarePHP.prototype.askForTravisAccount = function() {
	...
	var prompts = [{
			...
			default: this.config.get('controlRepository') ?
				this.config.get('accountTravis') : this.defaults.owner.canonical
		},
		{
			type: 'confirm',
			name: 'supportNightly',
...
BarePHP.prototype.askForCoverallsAccount = function() {
	...
	var prompts = [{
		...
		default: this.config.get('controlRepository') ?
			this.config.get('accountCoveralls') : this.defaults.owner.canonical
	}];
...
BarePHP.prototype.askForScrutinizerAccount = function() {
	...
	var prompts = [{
		...
		default: this.config.get('controlRepository') ?
			this.config.get('accountScrutinizer') : this.defaults.owner.canonical
	}];
...
BarePHP.prototype.writing = {
	...
	createDirs: function() {
		...
		// Create the source subfolders
        for (i = 0; i < this.config.get('dirSrcSubFolders').length; i++) {
            var entry = this.config.get('dirSrcSubFolders')[i];
            mkdirp(this.config.get('dirSrc') + '/' + entry);
	...
	writeFiles: function() {
		...
		// ADD PHP FILES
		this.template('../../templates/code/_.env', this.config.get('dirSrc') + '/.env');

            for (i = 0; i < this.config.get('dirSrcSubFolders').length; i++) {
                var entry = this.config.get('dirSrcSubFolders')[i];
    			var folder = this.config.get('dirSrc') + '/' + entry;
    			switch (folder) {
    				case 'src/App':
    					//Add Config & Routes
    					this.template('../../templates/code/_Config.php', folder + '/Config.php');
    					this.template('../../templates/code/_Routes.php', folder + '/Routes.php');
    					break;
    				case 'src/App/Controllers':
    					//Add HomeController
    					this.template('../../templates/code/_ControllerHome.php', folder + '/HomeController.php');
    					break;
    				case 'src/App/Models':
    					//Add UserModel
    					this.template('../../templates/code/_ModelUser.php', folder + '/User.php');
    					break;
    				case 'src/App/Views':
    					//Add layout
    					this.template('../../templates/code/_ViewLayout.html', folder + '/layout.html');
    					break;
    				case 'src/App/Views/Home':
    					//Add Home index
    					this.template('../../templates/code/_ViewHomeIndex.html', folder + '/index.html');
    					break;
    				case 'src/Core':
    					//Add CoreItems
    					this.template('../../templates/code/_CoreController.php', folder + '/Controller.php');
    					this.template('../../templates/code/_CoreRouter.php', folder + '/Router.php');
    					this.template('../../templates/code/_CoreView.php', folder + '/View.php');
    					this.template('../../templates/code/_CoreModel.php', folder + '/Model.php');
    					this.template('../../templates/code/_CoreError.php', folder + '/Error.php');
    					break;
    			}
            }
